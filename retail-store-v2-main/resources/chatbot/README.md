...
- username : **`dataworkz`**
- password : this is a shared read-only cluster; the current credential is published in the upstream MongoDB demo README at https://github.com/mongodb-industry-solutions/retail-store-v2
...
- An OpenAI or Azure OpenAI key for a gpt-4 class model (Dataworkz supports OpenAI, Azure OpenAI, and other providers)
...
This demo uses an OpenAI gpt-4 class model. Dataworkz also supports Azure OpenAI and other providers; configure your choice through the LLM Configuration screen.
...
- If you are using Azure OpenAI, select the Azure OpenAI deployment type in the LLM Configuration screen and provide your Azure endpoint, deployment name, and API key (see Dataworkz's LLM Configuration options for the exact field labels) -
```
Type: Generative
Deployment Type: AzureOpenAI
Model: <your gpt-4 deployment name>
Name: AzureOpenAI_gpt_4
...
User name: dataworkz
Password: <shared read-only credential, published in the upstream MongoDB retail-store-v2 README>
```
